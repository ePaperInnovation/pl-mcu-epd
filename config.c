...
int read_config(char *configfile, struct config *config)
{
    ...
    while (!stat)
    {
        ...
        if (strcmp(config_name, "display_type") == 0)
        {
            ...
            len = parser_read_str(&line[len], SEP, display_type,
                                  sizeof(display_type));
            // evaluate string
            strncpy(config->config_display_type, display_type,
                    sizeof(display_type));

            LOG("%s", display_type);
            if (strncmp(display_type, "D107", 4) == 0
                    || strncmp(display_type, "S047", 4) == 0)
            {
                // D107 and D107C
        ...
        else if (config->i2c_mode == NULL
                && strcmp(config_name, "i2c_mode") == 0)
        {
            char i2c_mode[32] = { 0, };
        ...
        else if (strcmp(config_name, "data_source") == 0)
        {
            ...
            len = parser_read_str(&line[len], SEP, data_source,
                                  sizeof(data_source));
            // evaluate string
        ...
        {
            char endianess[32];
            len = parser_read_str(&line[len], SEP, endianess,
                                  sizeof(endianess));
            // evaluate string
        ...
        else if (strcmp(config_name, "interface_type") == 0)
        {
            ...
            len = parser_read_str(&line[len], SEP, interface_type,
