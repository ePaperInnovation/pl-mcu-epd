...
unsigned int SectorErase(unsigned int addr) //RCL RESET
{//0x20 Sector erase
...
unsigned char ReadStatusRegister1(void) // RCL -RESET
{
...
void Read_Jedec(void)  // RCL - RESET
{
    unsigned int i;
...
void bitbang(unsigned char instr)   // RCL - RESET
{
    unsigned int i;
    P3OUT &= ~BIT3;     //clk=0;
    for(i=0; i<8; i++)
    {
        ...
        instr = instr <<1;
        __delay_cycles(50);
        P3OUT |= BIT3;      // clk=1 RCL
        __delay_cycles(50);
        P3OUT &= ~BIT3;     //clk=0;
        P3OUT &= ~BIT1;     // data to zero-low
        __delay_cycles(50);
    }
}


void bitbangL(unsigned char instr)  // RESET - RCL
{
    unsigned int i;
    P3OUT &= ~BIT3;     //clk=0;
    for(i=0; i<8; i++)
...
unsigned char CpuIn(void)  //RESET - RCL
{
    unsigned int i;
    unsigned char buf;
    unsigned char Indat;
